...
client = pymongo.MongoClient('mongodb+srv://priyapr785:RAGHUkala1@cluster0.hmliijy.mongodb.net/?retryWrites=true&w=majority')
db=client["youtube3"]
my_col=db["harvest3"]

con = pymysql.connect(host = 'localhost',
                        user = 'root',
                        password = "RAGHU@kala1"
                      )
...
api_key = 'AIzaSyDY1V7Solq7mGGX1yCVB2TV-bJiQq9hoVE'
...
def get_playlist_id(youtube,channel_id):
    ...
    while True:
        ...
        response = request.execute()
        for i in range(len(response['items'])):
            play = dict(playlist_id = response['items'][i]['id'],
                ...
                playlist_count = int(response['items'][i]['contentDetails']['itemCount']),
                playlist_publisheddate =str(datetime.datetime.strptime(response['items'][i]['snippet']['publishedAt'],"%Y-%m-%dT%H:%M:%SZ")))
...
def comment_details(youtube,ids):
    comments = []
    for video_id in ids:
        try:
...
if selected == "DATA COLLECTION":
    ...
    df = pd.DataFrame(
        {"Channel_Name" :["Lore So What","codebasics","Ken Jee","techTFQ","Tech Classes","Data With Mo","Data Professor ","Nicholas Renotte","Simplilearn","Krish Naik "],
        "Channel_ID" :["UCtoNXlIegvxkvf5Ji8S57Ag","UCh9nVJoWXmFb7sLApWGcLPQ","UCiT9RITQ9PW6BhXK0y2jaeg","UCnz-ZXXER4jOvuED5trXfEA","UCPvDKIsrjA_h3g5yZJwCIHA","UCDybamfye5An6p-j1t2YMsg","UCV8e2g4IWQqK71bbzGDEI4Q","UCHXa4OpASJEwrHrLeIzw7Yg","UCsvqVGtbbyHaMoevxPAq9Fg","UCNU_lfiiWBdtULKOw6X0Dig"]
        }     
                )
    st.data_editor(df)
